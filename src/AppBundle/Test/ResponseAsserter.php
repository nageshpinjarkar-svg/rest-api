 ...
 * 
 * Inside the new function, it just loops over each property 
 * and reads their value using the PropertyAccess component. 
 * And it is still just using json_decode internally. 
 * It's just an easier way to look into the JSON response.
 */
class ResponseAsserter extends \PHPUnit_Framework_Assert
{
    ...
    /**
     * Asserts the response JSON property same the given value
     * because assertResponsePropertyEquals don't work well 
     *
     * @param ResponseInterface $response
     * @param string $propertyPath e.g. firstName, battles[0].programmer.username
     * @param mixed $expectedValue
     * @throws \Exception
     */
    public function assertResponsePropertySame(ResponseInterface $response, $propertyPath, $expectedValue)
    {
        $actual = $this->readResponseProperty($response, $propertyPath);
        $this->assertSame(
            $expectedValue,
